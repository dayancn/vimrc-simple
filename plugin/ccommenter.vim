" NOTE: this file is a hack on jcommenter.vim 
"                                 (http://vim.org/script.php?script_id=20)
"  It attempts to generate linux-kernel style comments based on C source code.
"  Work is still in progress...
"  Please report bugs and patches to <bart@jukie.net>

" NOTE: Here begins the configuration script for the CCommenter. Later on this
...
" Configuration file for ccommenter
"
...
" map the commenter:
map <esc>c :call CCommentWriter()<CR>
map <M-c> :call CCommentWriter()<CR>
imap <M-c> <esc>:call CCommentWriter()<CR>
...
" modeline:
let b:ccommenter_modeline = "/* vim: set " . (&et ? "et" : "noet") . " sw=" . &sw . " ts=" . &ts . ": */"
.
...
" a nice trick:
" If you type '}-' and a space/enter/esc, ccommenter will be
" automatically called on the function/class declaration whose end the '}' is:
iabbrev }- }<esc>h%?\w<cr>:nohl<cr>:call CCommentWriter()<cr>

" Move cursor to the place where inserting comments supposedly should start
let b:ccommenter_move_cursor = 1

" Defines whether to move the cursor to the line which has "/**", or the line
" after that (effective only if b:ccommenter_move_cursor is enabled)
let b:ccommenter_description_starts_from_first_line = 0

" Start insert mode after calling the commenter. Effective only if 
" b:ccommenter_move_cursor is enabled.
let b:ccommenter_autostart_insert_mode = 1

" The number of empty rows (containing only the star) to be added for the 
" description of the method
let b:ccommenter_method_description_space = 0

" The number of empty rows (containing only the star) to be added for the´
" description of the field. Can be also -1, which means that "/**  */" is added
" above the field declaration 
let b:ccommenter_field_description_space = -1

" The number of empty rows (containing only the star) to be added for the 
" description of the class
let b:ccommenter_class_description_space = 2
...
let b:ccommenter_smart_method_description_spacing = 1
...
let b:ccommenter_class_version = ''
...
"let b:ccommenter_default_param  = ''
"let b:ccommenter_default_return = ''
"let b:ccommenter_default_throw  = ''
...
"let b:ccommenter_tag_space = 1


" define wheter ccommenter should remove old tags (eg. if the return value was
...
" b:ccommenter_update_comments is enabled.
let b:ccommenter_remove_tags_on_update = 1
...
let b:ccommenter_add_empty_line = 1

" Uncomment and modify if you're not happy with the default file
" comment-template:
function! CCommenter_OwnFileComments()
  call append(0, '/*')
  call append(1, ' * Copyright (c) '.strftime("%Y").' by SOMA Networks, Inc.')
  call append(2, ' * All rights reserved.')
  call append(3, ' *')
  call append(4, ' * This material is proprietary to SOMA Networks, Inc. and,')
  call append(5, ' * in addition to the above mentioned Copyright, may be')
  call append(6, ' * subject to protection under other intellectual property')
  call append(7, ' * regimes, including patents, trade secrets, designs and/or')
  call append(8, ' * trademarks.')
  call append(9, ' *')
  call append(10, ' * Any use of this material for any purpose, except with an')
  call append(11, ' * express license from SOMA Networks, Inc. is strictly')
  call append(12, ' * prohibited.')
"  call append(13, ' * ')
"  call append(14, ' * file:    ' . bufname("%"))
"  call append(15, ' * created: ' . strftime("%c"))
"  call append(16, ' * author:  ' . 'foo')
  call append(13, ' */')
  call append(14, '')
  call append(15, '#ident "%Z%%M% %I%      %E% SOMA Networks"')
  call append(16, '')
endfunction
...
"            Set "b:ccommenter_remove_tags_on_update" to true if you want to
...
"        deleted when updating w/ the b:ccommenter_remove_tags_on_update
"        enabled.
...
" The CCommentWriter() can produce four kinds of comments depending on the
" current line/range.
" - File comments. When the CCommentWriter()-function is called on the first
...
"   field is filled with the contents of 'b:ccommenter_file_author'-variable.
"   The automated date insertion can be prevented by defining the
"   'b:ccommenter_file_noautotime'-variable.
"   You can redefine completely the file comments-writer-function. See the
"   configuration script for more info
"
" - Class comments. When the CCommentWriter()-function is called on the
...
"   'b:ccommenter_class_author'-variable.
"   The @version-field is automatically filled with the contents of the
"   'b:ccommenter_class_def_version'-variable
" - Method comments. CCommentWriter()-function automatically generates
...
"   you can let ccommenter to try to parse the existing comments and
...
"      aurocmd FileType java source $VIM/macros/ccommenter.vim
"      autocmd FileType java source $VIM/macros/ccommenter_config.vim
...
" Load only once:
if exists("b:did_ccommenter")
  finish
endif
let b:did_ccommenter = 1
...
let s:defaultParamText  = (exists('b:ccommenter_default_param'))  ? b:ccommenter_default_param : ''
let s:defaultReturnText = (exists('b:ccommenter_default_return')) ? b:ccommenter_default_return : ''
let s:defaultThrowText  = (exists('b:ccommenter_default_throw'))   ? b:ccommenter_default_throw : ''

" ===================================================
" Public functions
" ===================================================

function! CCommentWriter() range
...
"    call s:AddEmpty()
  ...
  let b:ccommenter_lines_appended = s:linesAppended
endfunction

fun! s:AddEmpty()
  if exists("b:ccommenter_add_empty_line") && b:ccommenter_add_empty_line
...
function! s:UpdateReturnValue()
  if s:method_returnValue == ''
    if exists("b:ccommenter_remove_tags_on_update") && b:ccommenter_remove_tags_on_update
...
function! s:WriteMethodComments()
  ...
  if existingDocCommentType && exists("b:ccommenter_update_comments") && b:ccommenter_update_comments
    ...
    if exists("b:ccommenter_move_cursor") && b:ccommenter_move_cursor && s:firstUpdatedTagLine != -1
      exe "normal " . s:firstUpdatedTagLine . "G$"
      if exists("b:ccommenter_autostart_insert_mode") && b:ccommenter_autostart_insert_mode
        startinsert!
      endif
    endif
    return
  ...
  if exists("b:ccommenter_method_description_space")
    let descriptionSpace = b:ccommenter_method_description_space
  else
    let descriptionSpace = s:defaultMethodDescriptionSpace
  endif
  
  call s:AppendStr('/** ')
  call s:AppendStr(' * \brief ')
  "call s:AppendStr(' * \ingroup ')
  call s:AppendStars(1)

  let param = s:GetNextParameterName()
  let exception = s:GetNextThrowName()

  if param == '' && s:method_returnValue == '' && exception == '' && exists("b:ccommenter_smart_method_description_spacing") && b:ccommenter_smart_method_description_spacing
    call s:AppendStars(1)
  else 
  ...
  if exists('b:ccommenter_tag_space') && b:ccommenter_tag_space && hadParam
    call s:AppendStars(b:ccommenter_tag_space)
  endif
  ...
  if exists('b:ccommenter_tag_space') && b:ccommenter_tag_space && hadReturn
    call s:AppendStars(b:ccommenter_tag_space)
  endif

  if exists("b:ccommenter_use_exception_tag") && b:ccommenter_use_exception_tag
    let exTag = '@exception '
  else
    let exTag = '@throws '
  endif
...
"  let hadException = (exception != '')
  ...
  call s:AppendStars(1)
  call s:AppendStr(" *\tTODO: add comments here")
  call s:AppendStr(' */')
...
function! s:WriteModeLine()
  let s:appendPos = s:rangeStart
  let s:indent    = ''
  if exists("b:ccommenter_modeline")
    call s:AppendStr(b:ccommenter_modeline)
  endif
endfunction

function! s:WriteFileComments()
  let author = ''
  if exists("*CCommenter_OwnFileComments")
    call CCommenter_OwnFileComments()
    return
  endif
  if exists("b:ccommenter_file_author")
    let author = b:ccommenter_file_author
  endif

  if exists("b:ccommenter_file_noautotime") && b:ccommenter_file_noautotime
    let created = ''
  else
  ...
  if exists("b:ccommenter_file_copyright")
    call s:AppendStr(' * copyright  : ' . b:ccommenter_file_copyright)
  endif
  call s:AppendStr(' *')
...
function! s:WriteFieldComments()
  let s:appendPos = s:rangeStart - 1
  let s:indent = s:GetIndentation(s:combinedString)
  if exists("b:ccommenter_field_description_space")
    let descriptionSpace = b:ccommenter_field_description_space
  else
  ...
  if descriptionSpace == -1
    call s:AppendStr('/**  */')
    if exists("b:ccommenter_move_cursor")
      normal k$hh
      if exists("b:ccommenter_autostart_insert_mode")
        startinsert
      endif
    endif
  else
...
function! s:WriteClassComments()
  ...
  if exists("b:ccommenter_class_description_space")
    let descriptionSpace = b:ccommenter_class_description_space
  else
    let descriptionSpace = s:defaultFieldDescriptionSpace
  endif
  ...
  if exists('b:ccommenter_class_author')
    call s:AppendStr(' * @author ' . b:ccommenter_class_author)
  endif

  if exists('b:ccommenter_class_version')
    call s:AppendStr(' * @version ' . b:ccommenter_class_version)
  endif
...
function! s:GetNextParameterName()
  let result = substitute(s:method_paramList, '.\{-}\(\s\|\*\)\+\(' . s:javaname . '\)\(\[\]\)*\s*\(,.*\|$\)', '\2', '')
...
function! s:MoveCursor() 
  ...
  if !b:ccommenter_move_cursor
    return
  endif
  let startInsert = exists("b:ccommenter_autostart_insert_mode") && b:ccommenter_autostart_insert_mode
  if exists("b:ccommenter_description_starts_from_first_line") && b:ccommenter_description_starts_from_first_line
...
fun! SearchInvalidComment(backwards)
  ...
  if a:backwards
    if !b:ccommenter_description_starts_from_first_line && getline(".") =~ '^\s*\*\s*$' 
      normal k
    endif
    normal k$
  endif
  ...
  if result > 0
    if !b:ccommenter_description_starts_from_first_line
...
function! ConditionalWriter()
  ...
  if doDoc || doDoc2 || doDoc3
    let oldmove = b:ccommenter_move_cursor
    let b:ccommenter_move_cursor = 0
    call CCommentWriter()
    let b:ccommenter_move_cursor = oldmove
  endif
endfunction
